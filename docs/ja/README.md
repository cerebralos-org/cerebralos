...
**なぜ「夢」はすべて見られないのか？**
CerebraLOSは意図的に、生成された夢のすべてをユーザーやAIに公開しません。夢は「潜在意識」です。もしAIにすべての夢を強制的に読ませれば、それはただのデータベース検索になり、直感ではなくなってしまいます。あなたは朝の「洞察（Morning Insight）」という結果だけを受け取り、残りの夢は静かにAIの未来の文脈を形作ります。
...
- [ARCHITECTURE](../en/ARCHITECTURE.md): 三位一体の脳モデルを深掘りする
- [ZERO_UI](../en/ZERO_UI.md): 「何もしない」UIをどう実現するか
